        ...
        value: function render() {
            ...
            if (isHorizontal) {
                ...
                if (this.props.dynamicHeight) {
                    var itemHeight = this.getVariableHeight(this.state.selectedItem);
...
Carousel.propTypes = {
    ...
    resizingHeight: _propTypes2.default.boolean
};
...
var _initialiseProps = function _initialiseProps() {
    ...
    this.handleResize = function () {
        _this5.setState({ resizingHeight: true });
        _this5.updateSizes();
    };
    ...
    this.getVariableHeight = function (position) {
        var item = _this5.itemsRef && _this5.itemsRef[position];
        var images = item && item.getElementsByTagName('img');
        var divs = item && item.getElementsByTagName('div');
        if (_this5.state.hasMount && divs.length > 0) {
            return divs[0].offsetHeight;
        } else if (_this5.state.hasMount && images.length > 0) {
            var image = images[0];
